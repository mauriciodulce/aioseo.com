...
class WooCommerceProduct extends Product {
	...
	protected function getWooCommerceAggregateRating() {
		if (
			( ! method_exists( $this->product, 'get_reviews_allowed' ) || ! method_exists( $this->product, 'get_average_rating' ) ) ||
			( ! $this->product->get_reviews_allowed() || ! $this->product->get_average_rating() )
		) {
			return [];
		}
